...
src/backend/public/js/application.fi.min.js: $(frontend_js_files)
	NODE_ENV=production browserify -t reactify -t [ jedify --lang fi ] -t envify src/frontend/index.js | uglifyjs -c -m > $@

src/backend/public/js/application.en.min.js: $(frontend_js_files)
	NODE_ENV=production browserify -t reactify -t [ jedify --lang en ] -t envify src/frontend/index.js | uglifyjs -c -m > $@
